...
export default function WaterTracker() {
  ...
  if (pct >= 100) { hydrationGrade = "Excellent"; gradeColor = "text-emerald-500"; }
  else if (pct >= 75) { hydrationGrade = "Good"; gradeColor = "text-white"; }
  else if (pct >= 50) { hydrationGrade = "Fair"; gradeColor = "text-zinc-300"; }
  else if (pct >= 25) { hydrationGrade = "Low"; gradeColor = "text-zinc-500"; }
        ...
        <BorderGlow {...MONO_GLOW} className="w-full h-full">
          <div className="p-6 flex flex-col items-center text-center">
            <div className="flex items-center gap-2 mb-4 self-start">
              ...
              <h3 className="text-lg font-heading font-extrabold text-white">Hydration Monitor</h3>
            </div>
            ...
            <div className={`relative w-40 h-56 mb-6 transition-transform duration-300 ${animatePulse ? "scale-105" : ""}`}>
              <svg viewBox="0 0 100 140" className="w-full h-full">
                ...
                {/* Bottle background */}
                <path d="M30 20 L30 10 Q30 5 35 5 L65 5 Q70 5 70 10 L70 20 Q80 25 80 35 L80 125 Q80 135 70 135 L30 135 Q20 135 20 125 L20 35 Q20 25 30 20Z"
                  fill="none" stroke="currentColor" strokeWidth="2" className="text-zinc-700" />
                ...
                <text x="50" y="80" textAnchor="middle" className="fill-white" fontSize="16" fontWeight="bold" style={{ textShadow: "0 2px 4px rgba(0,0,0,0.5)" }}>
                  {pct}%
                </text>
              </svg>
            </div>
            ...
            <div className="mb-2">
              ...
              <span className="text-sm font-bold text-zinc-500 ml-1">/ {(target / 1000).toFixed(1)}L</span>
            </div>
            <div className={`text-xs font-bold ${gradeColor} mb-1 uppercase tracking-widest`}>{hydrationGrade}</div>
            <p className="text-xs font-semibold text-zinc-500 mb-6">{remaining > 0 ? `${remaining}ml remaining` : "🎉 Goal reached!"}</p>
            ...
            <div className="grid grid-cols-3 gap-2 w-full mb-4">
              {QUICK_AMOUNTS.map(({ label, value }) => (
                <button
                  ...
                  className="px-2 sm:px-3 py-3 rounded-2xl bg-zinc-900 hover:bg-white hover:text-black text-zinc-300 font-bold text-xs sm:text-sm transition-all border border-zinc-800"
                >
                  <Plus className="w-3 h-3 inline mr-1" />{label}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2 w-full">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-2xl bg-zinc-900 border border-zinc-800 outline-none text-sm font-bold text-white placeholder-zinc-500 focus:border-white transition-all shadow-inner"
              />
              <button
                onClick={handleCustomAdd}
                className="px-6 py-3 rounded-2xl bg-white text-black font-black text-sm shadow-lg shadow-white/20 hover:scale-105 transition-all"
              >
                Add
              </button>
            </div>
            ...
            <button
              onClick={handleReset}
              className="mt-4 w-full py-3 rounded-2xl bg-rose-500/10 text-rose-500 font-black text-xs hover:bg-rose-500/20 transition-colors border border-rose-500/20"
            >
              Reset Today's Log
            </button>
          </div>
        ...
        <BorderGlow {...MONO_GLOW} className="w-full">
          <div className="p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-lg font-heading font-extrabold text-white">Weekly Hydration</h3>
            </div>
              ...
              {weeklyData.map((val, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                    ...
                    <div className="w-full bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800" style={{ height: "100px" }}>
                      <div
                        className={`w-full rounded-2xl transition-all duration-500 shadow-lg ${
                          isToday ? "bg-white shadow-white/20" : "bg-zinc-600 shadow-none"
                        }`}
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-lg font-heading font-extrabold text-white">Today's Log</h3>
              </div>
            ...
            {log.length === 0 ? (
              <p className="text-sm font-medium text-zinc-500 text-center py-8">No water logged yet today. Start hydrating!</p>
            ) : (
              <div className="space-y-2.5 max-h-64 overflow-y-auto pr-1">
                {[...log].reverse().map((entry, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-3.5 rounded-2xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 transition-colors">
                    <div className="flex items-center gap-3">
                      <Droplet className="w-4 h-4 text-white" />
                      <span className="text-lg font-heading font-extrabold text-white">{entry.amount}ml</span>
                    </div>
